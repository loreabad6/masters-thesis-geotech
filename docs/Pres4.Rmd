...
lts_zones_facets <- rbind(
  cbind(lts_zones, faceter = "All observations"),
  cbind(lts_zones[lts_zones$bikeshare > 0,], faceter = "Zoom without 'zero' observations")
)

obs <- lts_zones_facets %>% 
  group_by(stress_level,faceter) %>% 
  summarize(
    n = paste("n = ", n()),
...
ggplot(lts_zones_facets, aes(x = bikeshare)) + 
  geom_histogram(
  ...
  facet_grid(faceter ~ stress_level, scales = "free_y") + 
  geom_text(
    data = obs,
    aes(x = 70, y = c(5000,300,5000,300), label = label),
    colour = "grey30",
    size = 5,
    family = "serif",
    parse = FALSE
