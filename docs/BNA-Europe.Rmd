...
**NOTE:** To run this command [set path variables](https://learnosm.org/en/osm-data/osm2pgsql/) for `osm2pgsql` and `osm2pgrouting` and [create a password file](https://www.postgresql.org/docs/current/libpq-pgpass.html) on `%APPDATA%/postgresql/pgpass.conf` with the format hostname:port:database:username:password. If the `%APPDATA%/postgresql/` directory does not exists, you can create it. 

```{r, echo = TRUE, include = FALSE, warning = FALSE}
system(
  command = paste0(
    "osm2pgsql -c -d ", db_name, 
    " -U ", user_name, 
    " -H ", local_host, 
    " -W --create --prefix sa_full -S ", 
    pfbstyle_file, " ", 
    osm_file, " --cache 600",
...
system(
  command = paste0(
    ...
    " -d ", db_name,
    " --username ", user_name,
    " --schema received --prefix sa_all_ --conf ",
    mapconfig_file,
    " --clean"),
  show.output.on.console = TRUE
)
...
system(
  command = paste0(
    ...
    " -d ", db_name,
    " --username ", user_name,
    " --schema received --prefix sa_bike_ --conf ",
    mapconfigbikes_file,
    " --clean"),
  show.output.on.console = TRUE
)
