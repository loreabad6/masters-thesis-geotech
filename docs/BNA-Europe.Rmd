...
**NOTE:** To run this command [set path variables](https://learnosm.org/en/osm-data/osm2pgsql/) for `osm2pgsql` and `osm2pgrouting` and [create a password file](https://www.postgresql.org/docs/9.1/libpq-pgpass.html) on %APPDATA%/postgresql/pgpass.conf with the format hostname:port:database:username:password. Also, change the paths to your local paths please :) I am trying to change this behavior

```{r, include = FALSE, warning = FALSE}
system(
  command = paste0("osm2pgsql -c -d bna_europe -U postgres -H localhost -W --create --prefix sa_full -S ", pfbstyle_file, " ", osm_file, "--cache 600",
...
if (!dbExistsTable(connection, c("received","geostat"))){
  ...
  dbWriteTable(
    conn = connection,
    name = Id(schema = "received", table = "pop_grid"),
    value = pop_grid_t
  )
  
  dbWriteTable(
    conn = connection,
    name = Id(schema = "received", table = "pop_table"),
    value = pop_table
  )
...
dbWriteTable(
  conn = connection,
  name = Id(schema = "generated", table = "sa_pop_grid"),
  value = sa_grid
)
```
